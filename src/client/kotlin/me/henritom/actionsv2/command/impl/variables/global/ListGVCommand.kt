package me.henritom.actionsv2.command.impl.variables.global

import com.mojang.brigadier.arguments.IntegerArgumentType
import com.mojang.brigadier.builder.LiteralArgumentBuilder
import me.henritom.actionsv2.variables.GlobalVariableStorage
import net.fabricmc.fabric.api.client.command.v2.ClientCommandManager
import net.fabricmc.fabric.api.client.command.v2.FabricClientCommandSource
import net.minecraft.text.Text

object ListGVCommand {

    fun register(): LiteralArgumentBuilder<FabricClientCommandSource>? {
        return ClientCommandManager.literal("list")
            .then(ClientCommandManager.argument("page", IntegerArgumentType.integer(1))
                .executes { context ->
                    val page = IntegerArgumentType.getInteger(context, "page")

                    val vars = GlobalVariableStorage.vars.toList()

                    context.source.sendFeedback(Text.translatable("actions.commands.variables.title", vars.size))

                    val maxPage = (vars.size / 10) + 1

                    context.source.sendFeedback(Text.translatable("actions.commands.variables.page", page.toString(), maxPage.toString()))

                    if (page > maxPage) {
                        context.source.sendError(Text.translatable("actions.commands.variables.page_not_found", page.toString()))
                        return@executes 0
                    }

                    val start = (page - 1) * 10
                    val end = start + 10

                    for (i in start until end) {
                        if (i >= vars.size)
                            break

                        val (name, value) = vars[i]

                        if (name.startsWith("color_") || name.startsWith("format_"))
                            context.source.sendFeedback(Text.translatable("actions.commands.list.it", name, value.toString() + "example"))
                        else
                            context.source.sendFeedback(Text.translatable("actions.commands.list.it", name, value.toString()))
                    }

                    1
                }
            )
    }
}